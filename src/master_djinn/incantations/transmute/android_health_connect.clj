(ns master-djinn.incantations.transmute.android-health-connect
  (:require [clojure.data.json :as json]
            [master-djinn.util.types.core :refer [action-type->name ->uuid]]))
...
(defn Step->Action
  ...
  (let [transmuter-version "0.0.1"
        ...
        start-time (:startTime input)
        origin (or (get-in input [:metadata :dataOrigin]) transmuter-data-provider) ]
    {:name action-name
    :player_relation "DID"
    :data {
      :uuid (gen-id origin action-name start-time transmuter-version)
...
(defn transmute
  ...
  (if (not= provider transmuter-data-provider)
    ...
    :actions (case action_name
      "Step" (map #(Step->Action % (->uuid pid provider)) inputs)
      "default" [])})))
